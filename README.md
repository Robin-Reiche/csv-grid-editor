...
- **Checkboxes for true/false columns** - Off by default, switched on with the gear at the right end of the toolbar. A column of `true/false`, `yes/no`, `y/n`, `t/f` or `on/off` values is drawn as checkboxes instead of words. One click flips a value. The file keeps its own words: the click writes back the opposite word of the pair that cell already used, in the same capitalisation, so `YES` becomes `NO`. Turning the mode on or off changes nothing in the file. An empty cell stays empty. Any value a box cannot stand for keeps its text.
- **Files without a header row** - Not every CSV starts with column names. Switch off **First row is the header** under the gear for such a file. The first line then shows as row 1 and the columns are named A, B, C. The choice is remembered for that file alone and changes nothing in it.
- **Settings menu** - The gear at the right end of the toolbar holds the small switches that are a matter of taste, in the editor rather than in the VS Code settings: **Color columns**, **Highlight the current row**, **Type badges in the header**, **Align numbers to the right**, **Mark empty cells**, **Checkboxes for true/false columns**, **Hide spaces around values** and **Enter moves down after editing**. Each one says in a line what it does and is remembered across files and sessions. None of them changes a value in the file. The gear is marked while anything in it differs from the default.
...
- **Insert & Delete rows and columns** - Right-click a row to insert above or below or to delete it, and right-click a column header to insert left or right or to delete. Select several rows (drag or `Shift`+click the `#` gutter) or several columns (`Shift`+click the headers) first to insert or delete that many at once, anchored to the selection edge, just like a spreadsheet. Three keys do the same without reaching for the mouse: `Ctrl+Enter` inserts a row below the one you are on, `Ctrl+Shift+Enter` inserts one above and `Ctrl+Shift+K` deletes it. They are the keys VS Code's own editor already uses for a line below, a line above and deleting a line. All three work while you are still editing a cell, an insert commits the value you typed first and a delete drops it along with the row, and with several rows selected they apply to the whole selection just like the context menu. A new, empty file shows **Add column** where the table would be, and a file with only a header row shows **Add row**, so a table can be started from nothing. `Ctrl+Enter` adds that first row too. Right-clicking the empty space beside or below the table offers **Add column** at the right end and **Add row** at the bottom.
...
- **Your file stays your file** - The grid writes a file back with its own line endings (CRLF, LF or CR), its final line break, its byte order mark and its encoding (UTF-8, UTF-16 or the Windows-1252 that Excel uses for plain "CSV"). Values keep the spaces around them. Quotes follow the CSV standard, so a value quoted without need loses its quotes the first time the file is written.
...
### Rename Columns
- **Rename** - Right-click a column header → **Rename column** to rename it. The new name is written to the CSV header row and is fully undoable. A file whose first row is not a header has nothing to rename, so the entry is not offered there.
...
### Auto-Reload
When a file is open in full (non-preview) mode, the editor watches the file on disk and **automatically reloads** the grid when the file is modified externally. This covers a plain rewrite as well as a script that replaces the file, for example one that deletes the output folder and regenerates the CSVs into it. Unsaved edits in the grid are never replaced: if the file changes while you have some, they stay and a warning offers **Reload from Disk** or **Overwrite**. Until you pick one, no save writes over the change, auto-save included. A value you are still typing in a cell counts as an unsaved edit.
