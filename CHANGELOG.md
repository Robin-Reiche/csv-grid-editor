...
- **Renaming or moving a file keeps its unsaved edits** - Renaming or moving a CSV in the Explorer while the grid had unsaved edits opened it again from disk. The edits were gone and the tab looked saved, so closing it asked nothing. The edits now go along to the new name and the tab stays marked unsaved, also after undoing the rename. A change on disk that was still waiting for a decision keeps waiting. A tab behind another shows the edits once it is brought to the front. A rename that fails leaves the tab marked unsaved. An edit made only in a Source Control diff opens in a grid tab of the new name.
